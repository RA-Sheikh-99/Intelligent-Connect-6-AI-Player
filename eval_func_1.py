...
def evaluate_board(board): # Evaluation function 1
    score = 0
    for y in range(BOARD_SIZE):
        for x in range(BOARD_SIZE):
            if board[y][x] == PLAYER2:
                score += evaluate_position(board, x, y, PLAYER2)
            elif board[y][x] == PLAYER1:
                score -= evaluate_position(board, x, y, PLAYER1)
    return score

def evaluate_position(board, x, y, player): # Evaluate score from a specific position
    score = 0
    directions = [(1, 0), (0, 1), (1, 1), (1, -1)]
    for dx, dy in directions:
        score += count_line(board, x, y, dx, dy, player)
    return score

def count_line(board, x, y, dx, dy, player): # Count consecutive stones in one line
    total = 0
    for i in range(-WIN_CONDITION + 1, WIN_CONDITION):
        nx = x + i * dx
        ny = y + i * dy
